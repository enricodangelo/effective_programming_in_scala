...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    val sortetGrades = grades.sortBy(_.ordinal)
    val gradeCount = sortetGrades.size
    val medianIndex = Math.ceil(gradeCount / 2).toInt
    return sortetGrades.apply(medianIndex)
end Grade
...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def elect(ballots: Seq[Ballot]): Candidate =
    ...
    val allGrades: Seq[(Candidate, Grade)] =
      ballots.flatMap((ballot: Ballot) => {
        val list: List[(Candidate, Grade)]
        ballot.grades.foreach {
          case(candidate: Candidate, grade: Grade) => {(candidate, grade)} ++ list
        }
        return list
      })
